...
class RDF_LidarExport
{
    ...
    // Format per sample: idx|hit|startX,startY,startZ|hitX,hitY,hitZ|dirX,dirY,dirZ|dist|colliderName|density|isWater
    // The trailing three fields are optional for backward compat (old clients
    // stop at dist). colliderName has |;,/ replaced with _ to stay parse-safe.
    static string SamplesToCSV(array<ref RDF_LidarSample> samples, bool compress = false, int decimalPlaces = 3)
    {
        ...
        // Collect per-sample CSV fragments into an array and join them in a
        // divide-and-conquer pass at the end. The old `csv += part + ";"`
        // accumulation was O(n²) in the total payload size (4096 rays × ~80B
        // ≈ 320KB/scan, ~670M char-copies/scan); the tree join is O(n log n).
        array<string> parts = new array<string>();
        parts.Reserve(samples.Count());
        for (int i = 0; i < samples.Count(); i++)
        {
            RDF_LidarSample s = samples.Get(i);
            // Format: idx|hit|startX,startY,startZ|hitX,hitY,hitZ|dirX,dirY,dirZ|dist|collider|density|isWater
            string hitStr = "0";
            if (s.m_Hit)
                hitStr = "1";
            ...
            // Sanitise collider name (drop chars that would break the wire format).
            // string.Replace mutates in place (returns void) in Enforce, so call
            // without reassigning.
            string coll = s.m_ColliderName;
            if (!coll)
                coll = "";
            coll.Replace("|", "_");
            coll.Replace(";", "_");
            ...
            // Resolve density / isWater from the surface material so the client
            // can shade by density without the GameMaterial pointer (which is
            // not serialised). Falls back to the sample's cached hints when the
            // surface is already gone.
            float density = s.m_Density;
            bool isWater = s.m_IsWater;
            if (s.m_Surface)
            {
                BallisticInfo bi = s.m_Surface.GetBallisticInfo();
                if (bi)
                {
                    float d = bi.GetDensity();
                    if (d >= 0.0)
                        density = d;
                    if (bi.IsWaterSurface())
                        isWater = true;
            ...
            if (isWater)
                isWaterStr = "1";
            part = part + "|" + coll + "|" + density.ToString() + "|" + isWaterStr;

            parts.Insert(part);
        }

        string csv = JoinStringParts(parts, ";");
    ...
    // Join an array of string fragments with a separator using a divide-
    // and-conquer merge so the total copy work is O(n log n) instead of the
    // O(n²) cost of repeated `csv += part + sep` accumulation. Each merge
    // level copies the full payload once; there are ceil(log2(n)) levels.
    static string JoinStringParts(array<string> parts, string sep)
    {
        int n = parts.Count();
        if (n == 0)
            return string.Empty;
        if (n == 1)
            return parts.Get(0);
        return JoinStringRange(parts, sep, 0, n);
    }

    protected static string JoinStringRange(array<string> parts, string sep, int lo, int hi)
    {
        if (lo + 1 == hi)
            return parts.Get(lo);
        int mid = (lo + hi) / 2;
        return JoinStringRange(parts, sep, lo, mid) + sep + JoinStringRange(parts, sep, mid, hi);
    }
    ...
    static array<ref RDF_LidarSample> ParseCSVToSamples(string csv)
    {
        ...
        foreach (string part : parts)
        {
            ...
            s.m_Dir = dir;
            s.m_Distance = dist;
            // Reconstruct m_End from start + dir * dist (ray endpoint). The
            // server serialises start/hitPos/dir/dist but not m_End, so the
            // client rebuilds it for visualisers that draw the full ray.
            s.m_End = start + dir * dist;

            // Optional extended fields (backward compat: old senders stop at dist).
            // Field 6 = colliderName, 7 = density, 8 = isWater.
            if (f.Count() >= 7)
                s.m_ColliderName = f.Get(6);
            if (f.Count() >= 8)
                s.m_Density = f.Get(7).ToFloat();
            if (f.Count() >= 9)
                s.m_IsWater = (f.Get(8) == "1");

            // m_Entity and m_Surface cannot be serialised across the network
            // (IEntity / GameMaterial are server-side handles). They stay null
            // on the client; the MaterialColorStrategy falls back to m_Density.
