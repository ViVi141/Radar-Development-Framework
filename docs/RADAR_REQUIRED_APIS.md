...
- **可见性**：对候选先可选 **DEM HEIGHT 预检**（`TryDemTerrainBlock`，默认开；地形挡则跳过 Trace），再 **TraceMove**（`RDF_RadarScanGeometry.TraceLineOfSight`：原点→目标代表点；`Flags=WORLD|ENTS|ANY_CONTACT`；`ExcludeArray`=subject+target；复用 TraceParam；每次清 TraceEnt/SurfaceProps）。`hitFraction ≥ 0.999` 或 `GetRootParent` 同层级 → 通视。
...
- **Visibility**: optional **DEM HEIGHT precheck** first (`TryDemTerrainBlock`, default on; terrain block skips Trace), then **TraceMove** via `RDF_RadarScanGeometry.TraceLineOfSight` (origin → representative aim point; `Flags=WORLD|ENTS|ANY_CONTACT`; `ExcludeArray`=subject+target; reused TraceParam; clear TraceEnt/SurfaceProps each cast). Clear when `hitFraction ≥ 0.999` or same `GetRootParent` hierarchy.
