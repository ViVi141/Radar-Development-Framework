...
- **座舱 range–az 杂波面**（`m_EnableRangeAzClutterSurface`，默认关）：预算化 DEM 射线填粗栅；Sensor Peek/Copy/PeekDb/CopyDb + `GetClutterSurfaceRevision` / `TryGetClutterSurfaceFill`；`RTTexture`→mesh `$rendertarget` + ×R⁴ LUT/扫线面板；角标 PPI 仍只画点迹
...
- **Cockpit range–az clutter surface** (`m_EnableRangeAzClutterSurface`, default off): budgeted DEM rays fill a coarse grid; Sensor Peek/Copy/PeekDb/CopyDb + revision/fill stats; `RTTexture`→mesh `$rendertarget` + ×R⁴ LUT/sweep panel; corner PPI stays plots-only
